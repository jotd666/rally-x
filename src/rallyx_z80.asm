...
; screen layout (intermixed with game variables!!):
;
; status right screen: 8 bytes every 32 bytes
; 0x8044-47: start of status right screen (4 first chars)
; 0x8040-43: end of status right screen (4 last chars)
; next row: +0x20
; +0x800: attributes
;
; main screen:
; 0x8400: top left (at start 0x8440)
; +0x20: next row
; +0x800: attributes
; wraps with scrolling
...
; bit 3: special flag picked up
; bit 2: flag picked up
...
; starting from here doesn't work, though
; (stack is probably not positioned properly)
...
01E5: D1          pop  de
01E6: E1          pop  hl
01E7: CD 6C 0E    call advance_hl_and_de_0e6c
...
042E: 00          nop
042F: 21 F4 89    ld   hl,sound_control_89f4
...
054F: 0E 70       ld   c,$70
0551: EB          ex   de,hl
0552: CD 4E 1C    call copy_status_row_1c4e
...
059A: D3 00       out  ($00),a
059C: 3E 49       ld   a,$49
059E: CD CB 1D    call clear_screen_and_reset_scroll_1dcb
05A1: 21 40 8C    ld   hl,$8C40
05A4: 06 90       ld   b,$90
05A6: 36 4A       ld   (hl),$4A		; [video_address]
05A8: 23          inc  hl
05A9: 10 FB       djnz $05A6
05AB: 21 00 8F    ld   hl,$8F00
05AE: 11 01 8F    ld   de,$8F01
05B1: 01 FF 00    ld   bc,$00FF
05B4: 36 47       ld   (hl),$47		; [video_address]
05B6: ED B0       ldir			; [video_address]
05B8: 21 F3 1F    ld   hl,$1FF3
05BB: FB          ei
05BC: CD 1B 1E    call write_instructions_text_1e1b
05BF: CD 1B 1E    call write_instructions_text_1e1b
05C2: CD 1B 1E    call write_instructions_text_1e1b
05C5: CD 1B 1E    call write_instructions_text_1e1b
...
05E9: 06 07       ld   b,$07
05EB: 36 44       ld   (hl),$44		; [video_address]
05ED: 23          inc  hl
...
06C8: 77          ld   (hl),a
06C9: 3E 66       ld   a,$66
06CB: CD CB 1D    call clear_screen_and_reset_scroll_1dcb
06CE: 21 30 1F    ld   hl,$1F30		; PUSH START BUTTON
06D1: 11 66 85    ld   de,$8566
06D4: CD 29 1C    call write_to_screen_1c29
06D7: E5          push hl
...
06E4: CD 29 1C    call write_to_screen_1c29
06E7: 2A B3 81    ld   hl,($81B3)
06EA: 01 07 00    ld   bc,$0007
06ED: 09          add  hl,bc
06EE: 11 F1 85    ld   de,$85F1
06F1: 0E 01       ld   c,$01
06F3: CD 2B 1C    call ldir_video_1c2b
06F6: 2A B3 81    ld   hl,($81B3)
06F9: 0E 04       ld   c,$04
06FB: CD 2B 1C    call ldir_video_1c2b
...
0778: 0E 66       ld   c,$66
077A: CD 4E 1C    call copy_status_row_1c4e
077D: 0E 72       ld   c,$72
...
079B: CD 4E 1C    call copy_status_row_1c4e
079E: 0E 72       ld   c,$72
07A0: 21 93 1F    ld   hl,$1F93
07A3: 10 03       djnz $07A8
07A5: 21 BB 1F    ld   hl,$1FBB
07A8: CD 08 0B    call $0B08
07AB: CD 4E 1C    call copy_status_row_1c4e
07AE: 21 BB 1F    ld   hl,$1FBB
07B1: CD 4E 1C    call copy_status_row_1c4e
07B4: AF          xor  a
...
0858: CD 4E 1C    call copy_status_row_1c4e
...
0A72: CD 6F 1D    call update_score_to_screen_1d6f
...
0ABE: AF          xor  a
0ABF: 32 23 80    ld   (double_score_8023),a
...
10C8: FB          ei
10C9: C3 62 11    jp   write_maze_rows_1162
...
1104: CD 1E 13    call write_maze_row_131e
1107: 14          inc  d
1108: 05          dec  b
1109: C8          ret  z
110A: E5          push hl
...
1138: 06 20       ld   b,$20
113A: CD 62 11    call write_maze_rows_1162
...
write_maze_rows_1162:
1162: C5          push bc
1163: D5          push de
1164: 06 20       ld   b,$20
1166: 2A 58 80    ld   hl,($8058)
1169: CD 1E 13    call write_maze_row_131e
116C: 1C          inc  e
116D: E5          push hl
...
12AA: F6 80       or   $80
12AC: 77          ld   (hl),a		; [video_address]
12AD: E1          pop  hl
12AE: C9          ret

write_flag_dots_12af:
...
12BF: CD 7F 12    call write_flag_dot_127f
...
1646: CD 69 0D    call increase_and_wrap_d5w_0d69
1649: 18 03       jr   $164E

164B: CD 5F 0D    call decrease_and_wrap_d5w_0d5f
...
179B: D1          pop  de
179C: E1          pop  hl
179D: CD 6C 0E    call advance_hl_and_de_0e6c
17A0: 0D          dec  c
...
17B1: 12          ld   (de),a
17B2: 05          dec  b
17B3: 21 F4 89    ld   hl,sound_control_89f4
17B6: 0E 04       ld   c,$04		; normal flag picked up sound
17B8: 20 07       jr   nz,$17C1
17BA: 0E 08       ld   c,$08		; double score flag picked up sound
17BC: 3E 01       ld   a,$01
17BE: 32 23 80    ld   (double_score_8023),a	; enable double score
17C1: 7E          ld   a,(hl)
17C2: B1          or   c
17C3: 77          ld   (hl),a
17C4: E1          pop  hl
...
17D7: CD AF 12    call write_flag_dots_12af
...
18D5: CD A3 17    call erase_chars_17a3
...
19A0: 21 F4 89    ld   hl,sound_control_89f4
19A3: 36 80       ld   (hl),$80
19A5: 23          inc  hl
19A6: 36 08       ld   (hl),$08
...
19C4: AF          xor  a
19C5: 32 F4 89    ld   (sound_control_89f4),a
...
1B1B: 23          inc  hl
1B1C: 46          ld   b,(hl)
1B1D: 12          ld   (de),a	; [video_address]
1B1E: 13          inc  de
1B1F: 10 FC       djnz $1B1D
1B21: 4E          ld   c,(hl)  
1B22: D1          pop  de
1B23: 23          inc  hl
1B24: ED B0       ldir		; [video_address]
1B26: C1          pop  bc
1B27: F1          pop  af
1B28: C9          ret
...
copy_status_row_1c4e:
1C4E: D5          push de
1C4F: C5          push bc
1C50: 01 08 00    ld   bc,$0008
1C53: ED B0       ldir			; [video_address]
1C55: C1          pop  bc
1C56: D1          pop  de
1C57: E5          push hl
1C58: D5          push de
...
1D3E: C8          ret  z
1D3F: 3A F4 89    ld   a,(sound_control_89f4)
1D42: E6 03       and  $03
1D44: F6 02       or   $02
1D46: 32 F4 89    ld   (sound_control_89f4),a		; play fuel countdown
1D49: AF          xor  a
...
1D61: CD 6F 1D    call update_score_to_screen_1d6f
1D64: C1          pop  bc
1D65: 05          dec  b
...
1DD4: 36 40       ld   (hl),$40	; [unchecked_address]
1DD6: ED B0       ldir		; [unchecked_address]
1DD8: 21 00 8C    ld   hl,$8C00
1DDB: 11 01 8C    ld   de,$8C01
1DDE: 01 FF 03    ld   bc,$03FF
1DE1: 77          ld   (hl),a	; [video_address]
1DE2: ED B0       ldir		; [video_address]
1DE4: AF          xor  a
...
1EF5: 77          ld   (hl),a	; [video_address]
1EF6: 78          ld   a,b
1EF7: A7          and  a
1EF8: 28 02       jr   z,$1EFC
1EFA: 2B          dec  hl    ; [uncovered] 
1EFB: 77          ld   (hl),a    ; [video_address]
1EFC: C1          pop  bc
1EFD: D1          pop  de
1EFE: E1          pop  hl
1EFF: C9          ret
...
2458: 36 01       ld   (hl),$01
245A: 3A F4 89    ld   a,(sound_control_89f4)
245D: CB 47       bit  0,a
...
247B: CB B6       res  6,(hl)
247D: CB BE       res  7,(hl)
247F: 21 F4 89    ld   hl,sound_control_89f4
2482: CB AE       res  5,(hl)
2484: C3 AF 24    jp   $24AF

2487: 21 0D 8A    ld   hl,$8A0D
248A: 36 00       ld   (hl),$00
248C: 3A F4 89    ld   a,(sound_control_89f4)
248F: CB 5F       bit  3,a
...
24C1: CB C6       set  0,(hl)
24C3: 21 F4 89    ld   hl,sound_control_89f4
24C6: 7E          ld   a,(hl)
24C7: 23          inc  hl
24C8: 46          ld   b,(hl)
24C9: F5          push af
...
254F: 3A F4 89    ld   a,(sound_control_89f4)
2552: E6 0E       and  $0E
2554: FE 00       cp   $00
...
256B: 3A F4 89    ld   a,(sound_control_89f4)
256E: CB 47       bit  0,a
...
2717: 21 F4 89    ld   hl,sound_control_89f4
271A: CB 86       res  0,(hl)
...
2751: 21 F4 89    ld   hl,sound_control_89f4
2754: FE 00       cp   $00
...
27A9: CB B6       res  6,(hl)
27AB: CB BE       res  7,(hl)
27AD: 21 F4 89    ld   hl,sound_control_89f4
27B0: CB A6       res  4,(hl)
27B2: C9          ret

handle_engine_sound_27b3:
27B3: 3A F4 89    ld   a,(sound_control_89f4)
27B6: CB 77       bit  6,a
...
27C0: 3A F4 89    ld   a,(sound_control_89f4)
27C3: CB 67       bit  4,a
...
boot_3800:
3800: 31 4C 38    ld   sp,boot_sequence_384C
...
38B7: 36 00       ld   (hl),$00
38B9: ED B0       ldir
38BB: CD 1E 39    call keep_clearing_screen_391e
38BE: 3E 01       ld   a,$01
...
39FC: E6 0E       and  $0E
39FE: FE 0C       cp   $0C
3A00: 38 02       jr   c,$3A04
3A02: D6 04       sub  $04
3A04: CB 41       bit  0,c
3A06: 20 01       jr   nz,$3A09
3A08: 3C          inc  a
3A09: FE 04       cp   $04
3A0B: 30 0F       jr   nc,$3A1C
3A0D: FE 02       cp   $02
3A0F: 38 0B       jr   c,$3A1C
3A11: 21 00 9C    ld   hl,$9C00
3A14: 11 00 84    ld   de,$8400
3A17: 01 00 04    ld   bc,$0400
3A1A: ED B0       ldir
3A1C: CB 3F       srl  a
3A1E: 32 CB 84    ld   ($84CB),a
3A21: 3E 4C       ld   a,$4C
3A23: 30 02       jr   nc,$3A27
3A25: 3E 48       ld   a,$48
